...
export const RobotHead = forwardRef<RobotHeadHandle, RobotHeadProps>(function RobotHead(
  ...
  // Increase mechanical clearance between head and base so pitch/yaw never penetrates or overlaps
  const jawY = -headH * 0.5 - jawH * 0.5 - 0.045 * scale;
  ...
  useFrame((state) => {
    ...
    // 1. Organic, persistent breathing float & subtle sway
    ...
    // 3. FLUID DECOUPLED KINEMATICS WITH PHYSICAL JOINT PROTECTION
    // The head tracks the pointer with full panoramic range.
    // The base acts as a floating gyroscopic chassis.
    // When the mouse moves to the left (ptrX < 0), left-side robots can now turn fully toward it.
    const headAimYaw = ptrX * 0.75;
    // Clamped pitch to completely eliminate base collision
    const headAimPitch = THREE.MathUtils.clamp(-ptrY * 0.22, -0.20, 0.18);
    const headAimRoll = ptrX * 0.06;

    // Base counter-reaction to pointer:
    // Gentle counter-reaction that stays synchronized and doesn't collide
    const baseCounterYaw = -ptrX * 0.08;
    const baseCounterPitch = THREE.MathUtils.clamp(ptrY * 0.05, -0.06, 0.06);
    const baseCounterRoll = -ptrX * 0.04;

    // Independent multi-frequency living idle motion
    const baseIdleYaw = Math.sin(now * 0.45 + phase * 0.8) * 0.05;
    const baseIdlePitch = Math.cos(now * 0.38 + phase * 1.1) * 0.03;
    const baseIdleRoll = Math.sin(now * 0.32 + phase * 0.6) * 0.02;

    const headIdleYaw = Math.sin(now * 0.72 + phase * 1.4) * 0.08 + Math.cos(now * 0.28 + phase * 2.2) * 0.04;
    const headIdlePitch = Math.sin(now * 0.62 + phase * 1.0) * 0.04;
    const headIdleRoll = Math.cos(now * 0.50 + phase * 1.7) * 0.03;

    // Floating spatial translation of root cluster
    const bodyLeanX = ptrX * 0.03 * scale;
    const bodyLeanY = ptrY * 0.02 * scale;
    ...
    // Fluent Base Rotation (Smooth, mechanical damping)
    ...
    // Fluent Head Rotation (Agile, full responsive swivel & pitch)
    if (headGroupRef.current) {
      // Dynamic vertical lift during pitch/swivel so head raises slightly above base on extreme angles
      const pitchLift = Math.abs(headAimPitch) * 0.04 * scale + Math.abs(headAimYaw) * 0.02 * scale;
      headGroupRef.current.position.y = pitchLift;

      const headTargetYaw = headAimYaw + headIdleYaw;
      const headTargetPitch = rotation[0] * 0.5 + headAimPitch + headIdlePitch;
      const headTargetRoll = rotation[2] * 0.5 + headAimRoll + headIdleRoll;

      headGroupRef.current.rotation.y = THREE.MathUtils.lerp(headGroupRef.current.rotation.y, headTargetYaw, 0.09);
      headGroupRef.current.rotation.x = THREE.MathUtils.lerp(headGroupRef.current.rotation.x, headTargetPitch, 0.09);
      headGroupRef.current.rotation.z = THREE.MathUtils.lerp(headGroupRef.current.rotation.z, headTargetRoll, 0.09);
    }
  ...
  return (
    <group ref={groupRef} position={position} rotation={rotation}>
      ...
      <group ref={baseGroupRef} position={[0, jawY, 0]}>
        ...
        <mesh position={[0, jawH * 0.5 + 0.018 * scale, 0]} material={socketMaterial}>
          <cylinderGeometry args={[0.13 * scale, 0.16 * scale, 0.040 * scale, 24]} />
        </mesh>
        {/* Machined spherical gimbal joint core visible during head pitch/yaw */}
        <mesh position={[0, jawH * 0.5 + 0.040 * scale, 0]} material={rivetMaterial}>
          <sphereGeometry args={[0.105 * scale, 20, 16]} />
        </mesh>
      </group>
      ...
      <group ref={headGroupRef} position={[0, 0, 0]}>
        ...
        <mesh position={[0, -headH * 0.5 - 0.008 * scale, 0]} material={gasketMaterial}>
          <cylinderGeometry args={[headW * 0.38, headW * 0.42, 0.024 * scale, 24]} />
        </mesh>
