...
export default function DemoModal({ isOpen, onClose, lang }: DemoModalProps) {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-xl bg-black border border-stone-800 rounded-2xl shadow-2xl p-6 sm:p-8 text-stone-200 overflow-hidden">
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute top-4 right-4 rtl:right-auto rtl:left-4 p-2 text-stone-400 hover:text-stone-100 hover:bg-stone-900 rounded-lg transition cursor-pointer"
        ...
        {!submitted ? (
          <div>
            <div className="flex items-center gap-2 text-xs font-mono text-[#e06b48] mb-2 uppercase tracking-wider font-semibold">
              ...
              <span>
                {lang === 'ar'
                  ? 'وصول مجاني كامل لمدة 14 يوماً'
                  : lang === 'fr'
                  ? 'Essai Gratuit 14 Jours • Sans Carte'
                  : 'Start for Free (14 Days) • No Card Required'}
              </span>
            </div>

            <h3 className="text-2xl font-editorial font-bold text-stone-100 mb-2">
              {lang === 'ar'
                ? 'ابدأ تجربة أتريوم مهيأً تماماً لنشاطك التجاري'
                : lang === 'fr'
                ? 'Activez Atrium configuré pour votre établissement'
                : 'Start your 14-day free trial configured for your trade'}
            </h3>

            <p className="text-xs text-stone-400 mb-6 leading-relaxed">
              {lang === 'ar'
                ? 'سنقوم بإعداد وتجهيز مساحة العمل لنشاطك مع صلاحيات كاملة وتدريب أولي بدون أي التزام مالي.'
                : lang === 'fr'
                ? 'Accès immédiat et complet à toutes les fonctionnalités d’Atrium adaptées à votre métier, sans engagement.'
                : 'Full unrestricted access to Atrium customized for your trade. Fast WhatsApp onboarding, zero card required.'}
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-left rtl:text-right transition cursor-pointer flex flex-col justify-between ${
                        businessType === item.id
                          ...
                          : 'bg-neutral-950/80 border-stone-850 text-stone-400 hover:text-stone-200 hover:border-stone-700'
                      }`}
                    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    <User className="w-4 h-4 text-stone-500 absolute left-3 rtl:left-auto rtl:right-3 top-3 pointer-events-none" />
                    <input
                      type="text"
                      required
                      ...
                      className="w-full pl-9 rtl:pl-3 rtl:pr-9 pr-3 py-2 text-xs bg-neutral-950 border border-stone-800 rounded-lg text-stone-100 placeholder-stone-600 focus:outline-hidden focus:border-[#e06b48]"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Building2 className="w-4 h-4 text-stone-500 absolute left-3 rtl:left-auto rtl:right-3 top-3 pointer-events-none" />
                    <input
                      type="text"
                      required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 text-xs bg-neutral-950 border border-stone-800 rounded-lg text-stone-200 focus:outline-hidden focus:border-[#e06b48]"
                  >
                ...
                <div>
                  ...
                  <div className="relative">
                    <Phone className="w-4 h-4 text-stone-500 absolute left-3 rtl:left-auto rtl:right-3 top-3 pointer-events-none" />
                    <input
                      type="tel"
                      required
              ...
              <p className="text-[11px] text-stone-500 text-center">
                {lang === 'ar'
                  ? 'بدون بطاقة دفع، لا التزام طويل الأجل. سنقوم بتجهيز حسابك والتواصل معك فوراً عبر واتساب.'
                  : lang === 'fr'
                  ? 'Sans carte bancaire, sans engagement. Configuration immédiate avec assistance WhatsApp.'
                  : 'Zero card required. We will configure your workspace and connect with you directly on WhatsApp.'}
              </p>
            </form>
          </div>
        ) : (
          <div className="py-8 text-center space-y-4">
            ...
            <h4 className="text-xl font-editorial text-stone-100 font-bold">
              {lang === 'ar' ? 'تم تفعيل طلب التجربة المجانية' : lang === 'fr' ? 'Demande d’essai bien reçue !' : 'Free 14-Day Trial Confirmed'}
            </h4>

            <p className="text-xs text-stone-400 max-w-sm mx-auto leading-relaxed">
              {lang === 'ar'
                ? `شكراً ${ownerName || 'لك'}. سيتواصل معك أحد مهندسينا عبر واتساب على الرقم (${phone}) لتسليمك بيانات الدخول وتجهيز أتريوم لـ ${businessName || 'مشروعك'}.`
                : lang === 'fr'
                ? `Merci ${ownerName || ''}. Un ingénieur va vous contacter via WhatsApp au (${phone}) pour vous donner vos accès et paramétrer Atrium pour ${businessName || 'votre établissement'}.`
                : `Thank you ${ownerName || ''}. An engineer will message you directly on WhatsApp at (${phone}) to provide your login credentials and setup Atrium for ${businessName || 'your business'}.`}
            </p>
