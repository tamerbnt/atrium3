...
import { ChevronLeft, ChevronRight, Pause, AlertCircle } from 'lucide-react';
...
interface ProblemCardSwiperProps {
  points: ProblemPoint[];
  autoPlayInterval?: number;
}

export function ProblemCardSwiper({
  points = [],
  autoPlayInterval = 4200,
}: ProblemCardSwiperProps) {
  ...
  if (count === 0) {
    return (
      <div className="w-full py-12 px-6 text-center text-stone-500 font-mono text-sm border border-stone-800/60 rounded-2xl bg-stone-950/40 my-6 flex flex-col items-center justify-center gap-2">
        <AlertCircle className="w-5 h-5 text-[#b85438]/80 mb-1" />
        <span>[ No operational bottlenecks currently logged ]</span>
      </div>
    );
  }
  ...
  return (
    <div
      ...
      tabIndex={0}
      aria-label="Infinite bottleneck problems carousel"
    >
              ...
              >
                <SpotlightCard
                  ...
                  className={`group p-6 sm:p-8 shadow-xl transition-all h-full flex flex-col justify-between ${
                    isCenterFocused
                      ? 'border-[#b85438]/80 ring-1 ring-[#b85438]/40 bg-[#0f0f13] shadow-[0_15px_45px_rgba(184,84,56,0.28)]'
                      : 'border-stone-800/85 bg-[#09090b]/80'
                  }`}
                >
                  <div>
                    <div className="flex items-center gap-2 text-xs font-mono mb-3.5 font-semibold">
                      <AlertCircle
                        className={`w-3.5 h-3.5 ${
                          isCenterFocused ? 'text-[#b85438]' : 'text-stone-500'
                        }`}
                      />
                      <span className={isCenterFocused ? 'text-[#b85438]' : 'text-stone-500'}>
                        [ 0{originalIdx + 1} — THE BOTTLENECK ]
                      </span>
                    </div>
                    ...
                    <p className="text-xs sm:text-sm text-stone-400 font-sans leading-relaxed min-h-[44px]">
                      {point.desc}
                    </p>
                  </div>

                  <div className="pt-4 border-t border-stone-800/80 text-[11px] font-mono text-stone-500 flex items-center justify-between mt-6">
                    <span className="text-stone-400">Pre-Atrium Operational Friction</span>
                    <span className={isCenterFocused ? 'text-[#b85438] font-bold' : 'text-stone-600'}>
                      {isCenterFocused ? 'Unresolved Overhead' : 'Click to Focus'}
                    </span>
                  </div>
                </SpotlightCard>
              </div>
      ...
      <div className="flex flex-col items-center justify-center gap-3 mt-4 px-4">
        {!isSingleItem && (
          <div
            ...
            role="tablist"
            aria-label="Problem cards pagination"
          >
            {points.map((_, idx) => (
              <button
                key={idx}
                ...
                aria-label={`Go to bottleneck ${idx + 1}`}
                onClick={() => goToOriginalIndex(idx)}
                className={`h-1.5 rounded-full transition-all duration-300 cursor-pointer ${
                  idx === activeOriginalIndex
                    ? 'w-8 sm:w-10 bg-[#b85438] shadow-[0_0_12px_rgba(184,84,56,0.95)]'
        ...
        <div className="flex items-center justify-center gap-2.5 text-xs font-mono text-stone-400">
          <span className="px-2 py-0.5 rounded bg-stone-900/90 border border-stone-800 text-[#b85438] font-bold text-[11px]">
          ...
          {!isSingleItem && (
            ...
            >
              {isAutoPlayEnabled && !isHovered ? (
                <span className="flex items-center gap-1 text-[10px] text-[#e06b48]/90 font-mono">
